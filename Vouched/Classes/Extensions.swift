...
extension UIImage {
    public convenience init?(pixelBuffer: CVPixelBuffer) {
        ...
        guard let ucgImage = cgImage else {
            return nil
        }

        self.init(cgImage: ucgImage)
    }
    
    func compressUntilSize(kB: Int) -> Data {
        let bytes = kB * 1024
        var compressionRate: CGFloat = 1.0
        var jpegCompressed = UIImageJPEGRepresentation(self, compressionRate)!
        
        while jpegCompressed.count > bytes {
            compressionRate = compressionRate - 0.05
            jpegCompressed = UIImageJPEGRepresentation(self, compressionRate)!
        }
        
        return jpegCompressed
    }
    
    func resize(withMaxResolution maxSize: CGFloat) -> UIImage {
        
        let maxSizeWithScale: CGFloat = maxSize / UIScreen.main.scale
        var finalWidth: CGFloat = self.size.width
        var finalHeight: CGFloat = self.size.height
        let highest: CGFloat = max(finalWidth, finalHeight)

        if highest > maxSizeWithScale {
            let reduceFactor = maxSizeWithScale / highest
            finalWidth = reduceFactor * finalWidth
            finalHeight = reduceFactor * finalHeight
        } else {
            return self
        }
